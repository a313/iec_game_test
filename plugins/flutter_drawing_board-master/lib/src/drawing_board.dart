  ...
  Widget build(BuildContext context) {
    return IconButton(
      ...
      icon: SvgPicture.asset(
        ...
        colorFilter: getColorByState(item),
      ),
    );
  }

  ColorFilter? getColorByState(DefToolItem item) {
    if (item.isActive && item.activeColor != null) {
      return ColorFilter.mode(item.activeColor!, BlendMode.srcIn);
    } else if (!item.isActive && item.color != null) {
      return ColorFilter.mode(item.color!, BlendMode.srcIn);
    }
    return null;
  }
}
